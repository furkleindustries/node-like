...
export abstract class AbstractNodeLike implements INodeLike {
  ...
  static strings = {
    CONTAINS_THIS_INVALID:
      'The implementing node did not meet the requirements of the ' +
      'isIParentNodeLike type guard.',

    CONTAINS_NODE_INVALID:
      'The node argument did not meet the requirements of the ' +
      'isIChildNodeLike type guard.',
  };

  contains(node: IChildNodeLike): boolean {
    if (!isIParentNodeLike(this)) {
      throw new Error(AbstractNodeLike.strings.CONTAINS_THIS_INVALID);
    } else if (!isIChildNodeLike(node)) {
      throw new Error(AbstractNodeLike.strings.CONTAINS_NODE_INVALID);
    }
      ...
      targetNode: INodeLike)
    {
      ...
      if (searchNode.isSameNode(targetNode)) {
        return true;
      } else {
